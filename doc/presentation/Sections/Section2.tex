...
\begin{columns}
  \begin{column}{0.55\textwidth}
    \footnotesize
   \begin{itemize}[<+>]
     \item \textbf{Émission} ($\sigma_e$) : Plus la température matière est élevée, plus l'émission est importante % Typiquement on ne vas pas retrouver sigma_e dans nos equations car on va se placer dans l'ETL, et on Planck.
     \item \textbf{Absorption} ($\sigma_a$) : Lorsqu'on est à l'équilibre thermique, $\sigma_a = \sigma_e$ % On va considerer en plus l'equilibre chimique ce qui donne l'ETL
     \item \textbf{Dispersion} ($\sigma_c$) : Il faut aussi tenir compte de la fonction de distribution angulaire de "scattering" $p(\bm{\Omega^\prime \rightarrow \bm{\Omega}})$ \parencite{Reference3}.
   \end{itemize}
  \end{column}
  % \pause
...
\begin{frame}
  \frametitle{L'ETR}
  L'équation du transfert radiatif est un bilan d'énergie lié au rayonnement au niveau mésoscopique (dans la direction $\bm{\Omega}$). 
    ...
    \begin{columns}
        ...
        \begin{column}{0.6\textwidth}
            \begin{figure}
                ...
                \only<2>{\caption{Des couches denses (Regression 2D)}}
            \end{figure}
        \end{column}
    \end{columns}
\end{frame}
