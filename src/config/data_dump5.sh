...
simulate () {
    ...
    echo -e "$rho$source$write_mode$simu_count" > src/config/part15.txt
    cat src/config/part15.txt src/config/part25.txt > src/config/tmp5.cfg
    build/transfer src/config/tmp5.cfg > /dev/null
}
