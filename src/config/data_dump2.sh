...
simulate () {
    ...
    echo -e "$rho$source$write_mode$simu_count" > src/config/part12.txt
    cat src/config/part12.txt src/config/part22.txt > src/config/tmp2.cfg
    build/transfer src/config/tmp2.cfg > /dev/null
}
