 ...
 */
void write_spatial(ofstream &file, vector<double> &signal, int n_rows, int n_cols){
    file << "\"[";
...
// ---probleme de free
// --- rajouter la reference
// --- conserver le pointeur
...
// ---------------------------------------------------------------------- AVEC solver
    ...
    //**************************** Parametres du probleme
    file << solver->mesh->x_min << "," << solver->mesh->x_max << "," << solver->mesh->y_min << "," << solver->mesh->y_max << "," << solver->mesh->N << "," << solver->mesh->M << "," << solver->c << "," << solver->a << "," << solver->C_v << ","<< solver->CFL << "," << solver->precision << "," << solver->t_0 << "," << solver->t_f << "," << solver->step_count << ",\"" << solver->rho_expr << "\",\"" << solver->sigma_a_expr << "\",\"" << solver->sigma_c_expr << "\",\"" << solver->E_0_expr << "\",\"" << solver->F_0_x_expr << "\",\"" << solver->F_0_y_expr << "\",\"" << solver->T_0_expr << "\",\"" << solver->E_u_expr << "\",\"" << solver->F_u_x_expr << "\",\"" << solver->F_u_y_expr << "\",\"" << solver->T_u_expr << "\",\"" << solver->E_d_expr << "\",\"" << solver->F_d_x_expr << "\",\"" << solver->F_d_y_expr << "\",\"" << solver->T_d_expr << "\",\"" << solver->E_l_expr << "\",\"" << solver->F_l_x_expr << "\",\"" << solver->F_l_y_expr << "\",\"" << solver->T_l_expr << "\",\"" << solver->E_r_expr << "\",\"" << solver->F_r_x_expr << "\",\"" << solver->F_r_y_expr << "\",\"" << solver->T_r_expr << "\",";

    //************************** Attributs du crenau de densite
    file << "\"[";
    for (int l = 0; l < solver->n_niche; l++){
        if (l != 0) file << ", ";
        file << "(" << solver->attr[l][0] << ", " << solver->attr[l][1] << ", " << solver->attr[l][2] << ", " << solver->attr[l][3] << ")";
    }
    file << "]\",";

    //************************** Densite
    file << "\"[";
    for (int j = 1; j <= solver->mesh->M; j++){
        file << "[";
        for (int i = 1; i <= solver->mesh->N; i++){
            file << (*solver).rho(solver->mesh->x[i], solver->mesh->y[j]);
            if (i != solver->mesh->N) file << ", ";
        }
        file << "]";
        if (j != solver->mesh->M) file << ", ";
    }
    file << "]\",";

    /* Donnes spatialles */
    write_spatial(file, solver->E, solver->mesh->N, solver->mesh->M);    file << ",";

    vector<double> F_x(solver->mesh->n_cells);
    vector<double> F_y(solver->mesh->n_cells);
    for (int k = 0; k < solver->mesh->n_cells; k++){
        F_x[k] = solver->F[k][0];
        F_y[k] = solver->F[k][1];
    }    
    write_spatial(file, F_x, solver->mesh->N, solver->mesh->M);    file << ",";
    write_spatial(file, F_y, solver->mesh->N, solver->mesh->M);    file << ",";

    write_spatial(file, solver->T, solver->mesh->N, solver->mesh->M);    file << ",";

    /* Donnees temporelles */
    write_temporal(file, solver->E_up, solver->step_count, solver->mesh->N);    file << ",";
    write_temporal(file, solver->F_up, solver->step_count, solver->mesh->N);    file << ",";
    write_temporal(file, solver->T_up, solver->step_count, solver->mesh->N);    file << ",";

    write_temporal(file, solver->E_down, solver->step_count, solver->mesh->N);    file << ",";
    write_temporal(file, solver->F_down, solver->step_count, solver->mesh->N);    file << ",";
    write_temporal(file, solver->T_down, solver->step_count, solver->mesh->N);    file << ",";

    write_temporal(file, solver->E_left, solver->step_count, solver->mesh->M);    file << ",";
    write_temporal(file, solver->F_left, solver->step_count, solver->mesh->M);    file << ",";
    write_temporal(file, solver->T_left, solver->step_count, solver->mesh->M);    file << ",";

    write_temporal(file, solver->E_right, solver->step_count, solver->mesh->M);    file << ",";
    write_temporal(file, solver->F_right, solver->step_count, solver->mesh->M);    file << ",";
    write_temporal(file, solver->T_right, solver->step_count, solver->mesh->M);    file << "\n";
